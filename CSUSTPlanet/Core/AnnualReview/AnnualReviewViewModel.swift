...
// MARK: - AnnualReviewData

struct AnnualReviewData {
    // MARK: - 个人信息
    /// 名字
    let name: String
    /// 名字拼音
    ...
    // MARK: - 成绩
    /// 年度总 GPA
    let annualGPA: Double
    /// 考试课程数量
    let examCount: Int
    /// 考查课程数量
    let assessmentCount: Int
    /// 满绩科目 (绩点 4.0)
    let fullGradePointCourses: [EduHelper.CourseGrade]
    /// 刚好及格科目 (绩点 1.0)
    let justPassedCourses: [EduHelper.CourseGrade]
    /// 挂科科目 (绩点 0.0)
    let failedCourses: [EduHelper.CourseGrade]
    /// 成绩最高分课程 (所有最高分课程)
    let highestGradeCourses: [EduHelper.CourseGrade]
    /// 年度总学分
    let totalCredits: Double
    /// 年度总课程数
    let totalCoursesCount: Int

    // MARK: - 课表
    /// 早八节次数量
    let earlyMorningCoursesCount: Int
    /// 各星期上课频次（字典：[星期几: 次数]）
    let dailyClassFrequency: [EduHelper.DayOfWeek: Int]
    /// 老师上课次数排行（字典：[老师名: 次数]）
    let teacherRanking: [String: Int]
    /// 周末上课次数
    let weekendCoursesCount: Int
    /// 晚课上课次数
    let eveningCoursesCount: Int
    /// 总学时（分钟）
    let totalStudyMinutes: Int
    /// 教学楼去往频次（字典：[建筑名: 次数]）
    let buildingFrequency: [String: Int]

    // MARK: - 网络课程平台
    /// 网络课程平台是否可用
    let moocAvailable: Bool
    /// 网络课程平台总在线时长（分钟）
    let moocTotalOnlineMinutes: Int?
    /// 网络课程平台登录次数
    let moocLoginCount: Int?

    struct DormStat: Identifiable, Hashable {
        var id: String { "\(campusName)-\(buildingName)-\(room)" }
        /// 校区名称
        ...
        let room: String
        /// 最低电量
        let minElectricity: Double
        /// 最高电量
        let maxElectricity: Double
        /// 充电次数
        let chargeCount: Int
        /// 查询次数
        let queryCount: Int
    ...
    /// 宿舍电量统计数据
    let dormElectricityStats: [DormStat]
}
...
final class AnnualReviewViewModel: ObservableObject {
    ...
    func compute() {
        Task {
            ...
            let maxGradeCourse = allGrades.max(by: { $0.grade < $1.grade })
            /// 成绩最高分课程 (所有最高分课程)
            let highestGradeCourses = allGrades.filter { $0.grade == maxGradeCourse?.grade }
            /// 年度总学分
            ...
            // MARK: - 网络课程平台
            /// 网络课程平台是否可用
            let moocAvailable = moocProfile != nil
            /// 网络课程平台总在线时长（分钟）
            let moocTotalOnlineMinutes = parseOnlineTime(moocProfile?.totalOnlineTime ?? "0小时0分")
            /// 网络课程平台登录次数
            ...
            let dormElectricityStats = dorms.compactMap {
                dorm -> AnnualReviewData.DormStat? in
                ...
                let chargeCount = zip(chronologicalRecords, chronologicalRecords.dropFirst())
                    ...
                    .count
                return AnnualReviewData.DormStat(
                    campusName: dorm.campusName,
                    buildingName: dorm.buildingName,
                    room: dorm.room,
                    minElectricity: minRecord.electricity,
                    maxElectricity: maxRecord.electricity,
                    chargeCount: chargeCount,
                    queryCount: dorm.records?.count ?? 0
                )
            }

            reviewData = AnnualReviewData(
                name: name,
                ...
                fullGradePointCourses: fullGradePointCourses,
                justPassedCourses: justPassedCourses,
                failedCourses: failedCourses,
                highestGradeCourses: highestGradeCourses,
                totalCredits: totalCredits,
                totalCoursesCount: totalCoursesCount,
                earlyMorningCoursesCount: earlyMorningCoursesCount,
                dailyClassFrequency: dailyClassFrequency,
                teacherRanking: teacherRanking,
                weekendCoursesCount: weekendCoursesCount,
                eveningCoursesCount: eveningCoursesCount,
                totalStudyMinutes: totalStudyMinutes,
                buildingFrequency: buildingFrequency,
                moocAvailable: moocAvailable,
                moocTotalOnlineMinutes: moocTotalOnlineMinutes,
                moocLoginCount: moocLoginCount,
                dormElectricityStats: dormElectricityStats
            )
        }
    }
